#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <inttypes.h>

#define RED_TEXT "\x1b[31m"
#define GREEN_TEXT "\x1b[32m"
#define RESET_COLOR "\x1b[0m"

#define SBOX_SIDE 16

extern uint8_t sbox_static_get_element(uint64_t index);
extern void sbox_dynamic(uint8_t *sbox);

uint8_t sbox_data[] = {
	};

int test_sbox_static()
{
	printf("Test sbox static\n");

	const uint64_t index = 255;
	const uint8_t result = sbox_static_get_element(index);
	printf("Here is an element: sbox[%" PRIu64 "] = 0x%02" PRIx8 "\n", index, result);

	for (int i = 0; i < SBOX_SIDE * SBOX_SIDE; i++)
	{
		const uint8_t elem_control = sbox_data[i];
		const uint8_t elem_returned = sbox_static_get_element(i);

		if (elem_control != elem_returned)
		{
			printf("sbox[%" PRIu8 "] should be 0x%02" PRIx8 " but returned 0x%02" PRIx8 "\n", i, elem_control, elem_returned);
			return 1;
		}
	}

	return 0;
}

int test_sbox_dynamic()
{
	printf("Test sbox dynamic\n");
	printf("Computed sbox (hex):\n");

	const size_t sbox_size = SBOX_SIDE * SBOX_SIDE * sizeof(uint8_t);
	uint8_t *sbox = (uint8_t *)malloc(sbox_size);
	if (sbox == NULL)
	{
		printf("Malloc sbox failed\n");
		return 1;
	}

	// Initialize sbox to 0
	for (int i = 0; i < SBOX_SIDE * SBOX_SIDE; i++)
	{
		sbox[i] = 0;
	}

	sbox_dynamic(sbox);

	// Print sbox
	for (int line = 0; line < SBOX_SIDE; line++)
	{
		for (int col = 0; col < SBOX_SIDE; col++)
		{
			printf("%02" PRIx8 " ", sbox[col + SBOX_SIDE * line]);
		}
		printf("\n");
	}

	// Test sbox
	for (int line = 0; line < SBOX_SIDE; line++)
	{
		for (int col = 0; col < SBOX_SIDE; col++)
		{
			const uint8_t elem_control = sbox_data[col + SBOX_SIDE * line];
			const uint8_t elem_returned = sbox[col + SBOX_SIDE * line];

			if (elem_control != elem_returned)
			{
				printf("sbox[%d] (line %d, column %d) should be 0x%02" PRIx8 " but returned 0x%02" PRIx8 "\n", col + SBOX_SIDE * line, line, col, elem_control, elem_returned);
				return 1;
			}
		}
	}

	free(sbox);
	return 0;
}

void print_return_status(int return_code)
{
	if (return_code == 0)
	{
		printf(GREEN_TEXT "success\n" RESET_COLOR);
	}
	else
	{
		printf(RED_TEXT "fail\n" RESET_COLOR);
	}
}

int main()
{
	print_return_status(test_sbox_static());

	printf("___________________________________\n");
	print_return_status(test_sbox_dynamic());

	return 0;
}
